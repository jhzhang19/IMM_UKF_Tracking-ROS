...
bool Load_Sensor_Data_Path(std::vector<std::string>& lidarfile_name, std::vector<std::string>& imagefile_name, string& path, string& file){
	string lidar_file_path = path + "/velodyne/" + file;
	cout<<lidar_file_path<<endl;
	if(!get_all_files(lidar_file_path, lidarfile_name))
		return false;
	string image_file_path = path + "/image_02/" + file;
...
int main(int argc, char** argv){
  ...
  image_transport::Publisher pubimage_ori = it.advertise("/send_data/image", 1);
  ...
  ros::Publisher pubtextmarker = nh.advertise<visualization_msgs::MarkerArray>("/mot_tracking/id", 1 );
	...
	//data path
	string datapath;
	ros::param::get("/tracking_node_ori/datapath", datapath);

	//data number
	string file;
	ros::param::get("/tracking_node_ori/file", file);

	//tracking class
	string trackclass;
	ros::param::get("/tracking_node_ori/trackclass", trackclass);
  //设定频率，在循环中通过ｓｌｅｅｐ达到控制频率的目的
	ros::Rate r(10);
  ...
  //加载点云和图片数据，获取点云和图案片文件的路径列表
	if(!Load_Sensor_Data_Path(lidarname, imagename, path, file)){
  ...
  string labelpath;
  ros::param::get("/tracking_node_ori/labelpath", labelpath);
  // std::string labelpath = "/home/zjh/workspace/kitti_track_dataset/tracking0020/label_02/0020.txt";
    ...
    cout<<"maxframe:"<<maxframe<<"\n"<<"lidarframe:"<<lidarname.size()<<endl;
	}
  else{
  ...
  //生成随机数
    ...
    // 目标的文字注释初始化
		...
		text_marker.lifetime = ros::Duration();//marker lifetime
		...
		text_marker.action = visualization_msgs::Marker::ADD;//# 0 add/modify an object, 1 (deprecated), 2 deletes an object, 3 deletes all objects
    ...
    //经纬度转ＵＴＭ坐标
		...
		//read image
		string impath = path + "/image_02/"+file+"/"+imagename[frame];
    cout<<"image path:"<<impath<<endl;
		cv::Mat rgbimage = cv::imread(impath);
    cv::Mat rgbimage_ori = rgbimage.clone();

		//read cloud
		string cloudpath = path + "/velodyne/"+file+"/"+lidarname[frame];
    ...
    sensor_msgs::ImagePtr imgmsg_ori = cv_bridge::CvImage(std_msgs::Header(), "bgr8",rgbimage_ori).toImageMsg(); //转换为ros接受的消息
		pubimage_ori.publish(imgmsg_ori);

		sensor_msgs::PointCloud2 ros_cloud;
		pcl::toROSMsg(*Cloud, ros_cloud);
    cout<<ros_cloud.width<<endl;
